...
- **이후에 작업할 내용 (미리보기)**
  - 초기에는 백엔드 연동 완료 시점을 MVP 완성으로 판단하였으나, 실제 논문 PDF를 기준으로 테스트를 진행하면서 수식 렌더링 안정성, 영어 텍스트 공백 처리, 이미지 포함 문서의 처리 성능 등 추가적으로 보완해야 할 요소들이 다수 확인되었다.
  - 이에 따라 본 고도화 단계에서는 ‘문서가 안정적으로 읽히는가’와 ‘문서의 구조적 특성을 정량적으로 표현할 수 있는가’를 핵심 기준으로 설정하고 기능을 재정리하였다.
  - 최종적으로는 문단 수, 수식 수, 이미지 수 등을 기반으로 문서 구조를 분석하고, 이를 바탕으로 복잡도 점수를 산출하는 기능까지를 이번 단계의 완료 범위로 설정하였다.
