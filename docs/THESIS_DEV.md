...
- **이후에 작업할 내용 (미리보기)**
  - “문서 구조를 분석하고 정량화하는 기능”을 추가
  목표: 논문 PDF를 읽어서 단순히 보여주는 것에 그치지 않고, 문서의 구조적 특성을 수치화하여 사용자에게 함께 제공하는 것

====================
[구현 목표]
====================

다음과 같은 문서 구조 분석 지표를 계산하고 표시해주세요.
...
7. 이미지 비율
8. 섹션별 길이 또는 비중
   - 예: Introduction / Method / Results / Conclusion
   - 섹션 제목이 명확하지 않으면 가능한 범위 내에서 추론 또는 탐지
9. 문단당 평균 길이
10. 수식이 많이 등장하는 구간 분포
    - 예: 문서 앞부분 / 중간 / 뒷부분
11. 이미지가 많이 등장하는 구간 분포
...
1. 현재 PDF 파싱/처리 파이프라인을 분석해서,
   어떤 단계에서 텍스트, 수식, 이미지 정보를 가장 안정적으로 얻을 수 있는지 파악한 뒤 구현해주세요.

2. 계산은 가능한 한 실제 파싱 결과를 기준으로 해주세요.
   단순 추정이 아니라, 현재 시스템에서 얻을 수 있는 데이터를 활용해 산출해주세요.

3. 수식, 이미지, 문단 카운트 기준을 코드상에서 명확히 정의해주세요.
   예:
   - 수식: block math + inline math + 특정 LaTeX 패턴
   - 이미지: PDF 내 embedded image 또는 추출된 figure
   - 문단: 줄바꿈/블록 구분 기준

4. 결과는 사용자 화면에서 보기 쉽게 표시해주세요.
   단순 JSON 출력 말고,
   문서 분석 요약 카드 또는 요약 섹션 형태로 렌더링해주세요.

5. 현재 프로젝트 구조를 크게 깨지 않는 최소 침습적 방식으로 구현해주세요.

====================
[UI 요구사항]
====================

문서 뷰어 상단 또는 사이드 영역에 “문서 분석 요약” 섹션을 추가해주세요.

표시 예시는 아래와 같은 형태면 좋습니다.
...
가능하면 카드형 UI 또는 간단한 시각화도 고려해주세요.
단, 과한 디자인보다는 깔끔하고 정보 전달이 잘 되는 형태를 우선해주세요.
...
3. 구간 분포 계산
   - 문서를 앞 1/3, 중간 1/3, 뒤 1/3으로 나누어
     수식과 이미지가 어느 구간에 많이 분포하는지 계산

4. 섹션 추출
   - 헤더 패턴(예: Introduction, Method, Results, Conclusion, Abstract 등)을 우선 탐지
   - 섹션 탐지가 어렵다면 현재 파싱된 구조를 바탕으로 가능한 수준에서 근사값 제공
   - 섹션 탐지가 불완전한 경우 그 한계도 코드/설명에 명시

====================
[코드 품질 요구사항]
====================

1. 단순히 한 파일에 다 몰아넣지 말고,
   분석 로직은 별도 유틸/서비스 함수로 분리해주세요.

2. 추후 확장 가능하도록 구조를 잡아주세요.
   예:
   - 문서 분석 서비스
   - 페이지별 메타데이터 계산기
   - 섹션 추출기

3. 타입이 있다면 타입도 정리해주세요.
   예:
...
현재 테스트 PDF가 아래처럼 3종 있다고 가정하고,
가능하면 이 기준으로 분석 결과가 잘 나오는지 확인해주세요.
...
- 페이지별 분포
- 전체 통계
가 직관적으로 잘 구분되는지 확인해주세요.

중요:
- 단순 아이디어 제안이 아니라 실제 코드 수정까지 진행해주세요.
- 기존 PDF 처리 파이프라인과 현재 렌더링 구조를 분석한 후,
  가장 자연스럽게 붙는 방식으로 구현해주세요.
- 이 기능은 “논문을 보기 좋게 보여주는 것”을 넘어서,
  “문서 구조를 데이터처럼 읽고 분석하는 기능”이 되도록 해주세요.