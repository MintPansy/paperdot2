...
- **이후에 작업할 내용 (미리보기)**
  - 초기에는 4월 2~3일, 문서함 PDF 즉시 표시 구조 완성 및 Vercel·Railway 풀스택 배포가 완료된 시점을 MVP 완성으로 판단하였으나, 실제 논문 PDF를 기준으로 테스트를 진행하면서 수식 렌더링 안정성, 영어 텍스트 공백 처리, 이미지 포함 문서의 처리 성능 등 추가적으로 보완이 필요한 요소들이 다수 확인되었다.
  - 이에 따라 본 고도화 단계에서는 다음 두 기준을 독립적으로 설정하고 기능을 재정리하였다.
    - **렌더링 품질**: 실제 논문 PDF가 수식·텍스트·이미지 구분 없이 안정적으로 읽히는가
    - **정량 분석**: 문서의 구조적 특성(문단 수·수식 수·이미지 수)을 수치로 표현할 수 있는가
  - 최종적으로는 수식 렌더링 안정화와 문단 수·수식 수·이미지 수 기반 복잡도 점수 산출 기능 구현까지를 이번 단계의 완료 범위로 설정하였다.
  - 향후에는 복잡도 점수 산출 기준의 정교화, 논문 핵심 정보의 구조화 추출, 대용량 문서 처리 성능 개선 등의 방향으로 확장이 가능하나, 이는 본 단계의 핵심 목표를 넘어서는 영역이므로 후속 개발 과제로 분리하였다.